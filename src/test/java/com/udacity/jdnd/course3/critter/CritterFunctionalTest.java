...
public class CritterFunctionalTest {
    ...
    static void initTest() throws SQLException {
        Server.createWebServer("-web", "-webAllowOthers", "-webPort", "8081","-webDaemon")
                .start();
    }
    @Test
    ...
    public void testAddPetsToCustomer() {
        CustomerDTO customerDTO = createCustomerDTO();//only name and phone number
        CustomerDTO newCustomer = customerController.saveCustomer(customerDTO);

        PetDTO petDTO = createPetDTO(); //only name and type
        petDTO.setOwnerId(newCustomer.getId()); //set ownerId
        PetDTO newPet = petController.savePet(petDTO);  //save and return newPet(name, type, ownerId) with the saved and Generated pet's id
